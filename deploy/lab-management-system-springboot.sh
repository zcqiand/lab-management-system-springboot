...
if [ ! -f "$BASE/springboot.env" ]; then
  if [ -n "${DATABASE_URL:-}" ] && [ -n "${DATABASE_USER:-}" ] && [ -n "${DATABASE_PASSWORD:-}" ] && [ -n "${JWT_SIGNING_KEY:-}" ] && [ -n "${LAB_SAAS_CLIENT_SECRET:-}" ]; then
    ...
    umask 077
    {
      ...
      # dev-password 值=契约值(.env.production 明文,demo 目录口令非真 secret);Secrets 可覆盖。
...
if [ -f "$BASE/springboot.env" ]; then
  ...
  # JWT_REFRESH_TTL_SECONDS 数值契约值直接补;dev-password 值=契约值(.env.production
  # 明文,demo 目录口令非真 secret),Secrets 可覆盖。
  append_if_missing JWT_REFRESH_TTL_SECONDS '604800'
  append_if_missing LAB_AUTH_DEV_PASSWORD "${LAB_AUTH_DEV_PASSWORD:-dev123456}"
