...
/**
 * V004__init_inspection_catalog.sql — 牌号码表（M04.F09）。PK = (tenant_id, code) per V012。list/create 必带
 * tenant_id；technical_requirements.brand FK ON DELETE SET NULL，删除允许（F06 决定，不强制引用校验）。
 */
@Entity
@Table(name = "inspection_brands")
@IdClass(CatalogEntryKey.class)
public class InspectionBrandEntity {
